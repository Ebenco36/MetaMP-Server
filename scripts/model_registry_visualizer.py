...
Usage — standalone:
    python model_registry_visualizer.py \
        ...
        --out /tmp/metamp_model_registry_figures

Usage — as stdin payload via docker exec:
    docker exec -i <container> python - --csv /var/app/data/models/production_ml/tables/model_bundle_registry.csv --out /tmp/metamp_model_registry_figures < model_registry_visualizer.py
    ...
    def from_dict(cls, row: dict[str, Any]) -> "ModelRecord":
        ...
        if isinstance(raw, str):
            try:
                ...
                features = [f.strip().strip('"') for f in raw.strip("[]").split(",") if f.strip()]
        else:
            features = list(raw)
        ...
        return cls(
            ...
            expert_precision_weighted_exact=_float(row.get("expert_precision_weighted_exact")),
            expert_recall_weighted_exact=_float(row.get("expert_recall_weighted_exact")),
...
class ModelRegistryVisualizer:
    ...
    def _enrich_df(self) -> None:
        ...
        numeric_cols = [
            ...
            "cv_mean_f1",
            "expert_accuracy",
            "expert_precision_weighted",
            "expert_recall_weighted",
            "expert_f1_weighted",
            "expert_accuracy_exact",
            "expert_precision_weighted_exact",
            "expert_recall_weighted_exact",
            "expert_f1_weighted_exact",
        ]
    ...
    def display_summary(self) -> None:
        ...
        if upload_sel:
            ...
            print(
                f"     Expert F1 : {r.expert_f1_weighted:.4f}  |  "
        ...
        top5 = sorted(
            self.records,
            key=lambda r: r.expert_f1_weighted,
            reverse=True,
        )[:5]
        print(f"\n{sep}")
        print("  TOP 5 BY EXPERT F1")
        hdr = (
            f"  {'Rank':<5} {'Artifact ID':<55} {'Mode':<16} {'DR':<8} "
            f"{'Expert F1':>10} {'CV F1':>8} {'Gap':>8}"
        )
        print(hdr)
        ...
        for i, r in enumerate(top5, 1):
            ...
            print(
                f"  {i:<5} {r.artifact_id:<55} {r.training_mode:<16} "
        ...
        for metric in metrics:
            if metric in df.columns:
                ...
                print(
                    f"  {metric:<32} {series.mean():>8.4f} {series.median():>8.4f} "
    ...
    def display_record(self, artifact_id: str) -> None:
        ...
        print(sep)
        rows = [
            ("Training mode", record.training_mode),
            ("Reduction", record.reduction_key),
            ("Classifier", record.classifier_name),
            ("Selected upload", record.selected_for_upload),
            ("Selected mode", record.selected_for_mode),
            ("CV Accuracy", f"{record.cv_mean_accuracy:.4f}"),
            ("CV Precision", f"{record.cv_mean_precision:.4f}"),
            ("CV Recall", f"{record.cv_mean_recall:.4f}"),
            ("CV F1", f"{record.cv_mean_f1:.4f}"),
            ("Expert rows", record.expert_row_count),
            ("Expert Accuracy", f"{record.expert_accuracy:.4f}"),
            ("Expert Precision", f"{record.expert_precision_weighted:.4f}"),
            ("Expert Recall", f"{record.expert_recall_weighted:.4f}"),
            ("Expert F1", f"{record.expert_f1_weighted:.4f}"),
            ("Exact Accuracy", f"{record.expert_accuracy_exact:.4f}"),
            ("Exact Precision", f"{record.expert_precision_weighted_exact:.4f}"),
            ("Exact Recall", f"{record.expert_recall_weighted_exact:.4f}"),
            ("Exact F1", f"{record.expert_f1_weighted_exact:.4f}"),
            ("CV→Expert Gap", f"{record.cv_expert_f1_gap:+.4f}"),
            ("Features", ", ".join(record.feature_columns)),
        ]
    ...
    def fig_cv_vs_expert_scatter(self) -> go.Figure:
        ...
        df["hover"] = (
            df["artifact_id"]
            + "<br>CV F1: "
            + df["cv_mean_f1"].round(3).astype(str)
            + "<br>Expert F1: "
            + df["expert_f1_weighted"].round(3).astype(str)
            + "<br>Mode: "
            + df["training_mode"]
        )
        fig = go.Figure()
        ...
        if not upload.empty:
            ...
            fig.add_annotation(
                x=row["cv_mean_f1"],
                y=row["expert_f1_weighted"],
                text=f"  ★ {row['artifact_id'].replace('_', ' ')}",
                showarrow=True,
                arrowhead=2,
        ...
        fig.update_xaxes(
            title_text="Cross-Validation F1",
    ...
    def fig_grouped_bar_classifier_mode(self) -> go.Figure:
        ...
        fig.update_xaxes(
            title_text="Mean Expert F1 (Weighted)",
            range=[0.72, 0.94],
            tickformat=".2f",
            showgrid=True,
            gridcolor=GRID_COLOR,
    ...
    def fig_heatmap_classifier_dr(self) -> go.Figure:
        ...
        for i, row_label in enumerate(pivot.index):
            for j, col_label in enumerate(pivot.columns):
                ...
                text = "n/a" if pd.isna(val) else f"{val:.3f}"
                font_color = FONT_COLOR_MU if pd.isna(val) else ("white" if val < 0.84 else FONT_COLOR_FG)
                annotations.append(
                    dict(
                        x=j,
                        y=i,
    ...
    def fig_cv_expert_gap_boxplot(self) -> go.Figure:
        ...
        for dr_key in dr_keys:
            ...
            fig.add_trace(
                go.Box(
                    y=sub,
                    name=DR_DISPLAY.get(dr_key, dr_key),
                    marker_color=colour,
                    boxmean="sd",
                    line=dict(width=1.8),
                    width=0.45,
                    hovertemplate=DR_DISPLAY.get(dr_key, dr_key) + "<br>Gap: %{y:.4f}<extra></extra>",
                )
            )

        fig.add_hline(
            y=0,
            line=dict(color="#a12c7b", width=1.4, dash="dash"),
            annotation_text="CV = Expert",
    ...
    def fig_bubble_cv_expert(self) -> go.Figure:
        ...
        for clf, colour in CLASSIFIER_PALETTE.items():
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    text=sub["artifact_id"]
                    + "<br>Exact Acc: "
                    + sub["expert_accuracy_exact"].round(3).astype(str),
                    hovertemplate="<b>%{text}</b><br>CV F1: %{x:.4f}<br>Expert F1: %{y:.4f}<extra></extra>",
                )
            )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=[0.70, 1.00],
                mode="lines",
                line=dict(color=GRID_COLOR, width=1.2, dash="dot"),
                name="Identity",
                showlegend=True,
            )
        )
    ...
    def fig_parallel_coordinates(self) -> go.Figure:
        ...
        df = self.df.copy()
        df["mode_idx"] = df["training_mode"].map(
            {"supervised": 0, "semi_supervised": 1}
        ).fillna(0)
        metrics = [
            ("cv_mean_f1", "CV F1"),
        ...
        dimensions = [
            dict(range=[0.70, 1.00], label=label, values=df[col].fillna(0).tolist())
    ...
    def fig_dr_subplot_comparison(self) -> go.Figure:
        """1×4 subplot panel — Expert F1 per DR method, colour = classifier."""
        df = self.df.copy()
        dr_keys = self._registry_dr_keys()
        ...
        layout["height"] = max(FIGURE_HEIGHT, 70 * max_rows + 200)
        fig.update_layout(
            **layout,
        )
        fig.update_xaxes(
    ...
    def _export_figure(
        ...
        paths: dict[str, Any] = {"paths": {}, "errors": {}}
        for fmt in formats:
            ...
            try:
                ...
                paths["errors"][fmt] = str(exc)
        return paths

    def render_all(
        self,
        output_dir: str | Path = "/tmp/metamp_model_registry_figures",
        formats: tuple[str, ...] = ("png", "pdf", "html", "json"),
        top_n: int = 15,
    ) -> dict[str, Any]:
        ...
        for stem, builder in builders:
            ...
            fig = builder()
            export_info = self._export_figure(fig, stem, out, formats=formats)
            manifest["figures"][stem] = export_info
            if "pdf" in formats and "pdf" not in export_info["paths"]:
                error_message = export_info["errors"].get("pdf") or "PDF export was not produced."
                raise RuntimeError(
                    f"Model registry figure '{stem}' is missing its PDF export: {error_message}"
                )
...
def _cli() -> None:
    ...
    parser.add_argument(
        "--out",
        default="/tmp/metamp_model_registry_figures",
        help="Output directory for rendered figures",
    )
    parser.add_argument(
        "--formats",
        nargs="+",
        default=["png", "pdf", "html", "json"],
