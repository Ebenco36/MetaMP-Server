...
class PDBDatabase(Migration):
    ...
    def generate(csv_path, output_file='model.py'):
        ...
        for column_name, dtype in zip(df.columns, df.dtypes):
            ...
            if not hasattr(PDB, short_column_name):
                setattr(PDB, short_column_name, db.Column(column_type))
                
        # for column_name, dtype in zip(df.columns, df.dtypes):
        #     column_type = Migration.get_column_type(dtype)
        #     if not hasattr(PDB, Migration.shorten_column_name(column_name)):
        #         setattr(PDB, Migration.shorten_column_name(column_name), db.Column(column_type))
...
def generate_model_class_PDB(csv_path, output_file='model.py'):
    return PDBDatabase().generate(csv_path, output_file)