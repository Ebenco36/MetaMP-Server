...
def get_all_pdb_codes():
    ftp_url = "ftp://ftp.wwpdb.org/pub/pdb/derived_data/pdb_entry_type.txt"
    
    try:
        with urllib.request.urlopen(ftp_url) as response:
            lines = response.read().decode().split('\n')
            
            # Filter out empty lines and extract PDB codes
            pdb_codes = [line.split()[0] for line in lines if line.strip()]
            
            return pdb_codes
    except Exception as e:
        print("Error fetching PDB codes:", e)
        return []
...
class PDBOTHERSJOBS:
    ...
    def load_data(self):
        ...
        check_quant_file = does_file_exist(modified_path + "/datasets/pdb/PDB_data.csv")
        if(check_quant_file):
            print(f"Error: File /datasets/pdb/PDB_data.csv already downloaded. You can delete to download new one.")
            return
        #Read the ids from the mpstruc
        current_date = datetime.date.today().strftime('%Y-%m-%d')
        self.ids = pd.read_csv(modified_path + "/datasets/all_only_from_pdb.csv")
        protein_entries = []
        # Load others
    ...
    def parse_data(self):
        try:
            ...
            if(not check_quant_file):
                ...
                data.to_csv(modified_path + "/datasets/pdb/PDB_data.csv", index=False)
                return self
            else:
    ...
    def loadFile(self):
        check_quant_file = does_file_exist(modified_path + "/datasets/pdb/PDB_data_transformed.csv")
        if(not check_quant_file):
            current_date = datetime.date.today().strftime('%Y-%m-%d')
            file_path = modified_path + '/datasets/pdb/PDB_data.csv'
            ...
            merged_df.to_csv(modified_path + '/datasets/pdb/PDB_data_transformed.csv', index=False)
        else:
            merged_df = pd.read_csv(modified_path + "/datasets/pdb/PDB_data_transformed.csv", low_memory=False)
            # merged_df = self.dataPrePreprocessing(merged_df)
            merged_df.to_csv(modified_path + '/datasets/pdb/PDB_data_transformed.csv', index=False)
        return merged_df
...
all_pdb_codes = get_all_pdb_codes()

# remove current MPstruc DB
MPstruc_db = pd.read_csv(modified_path + "/datasets/mpstruct_ids.csv")
print("Total number of PDB codes:", len(all_pdb_codes))
all_pdb_codes = list(map(lambda x: x.upper(), all_pdb_codes))
leftover = set(all_pdb_codes) - set(MPstruc_db["Pdb Code"].to_list())
df = pd.DataFrame({'PDB Code': list(leftover)})
df.to_csv(modified_path + "/datasets/all_only_from_pdb.csv")

pdb_obj = PDBOTHERSJOBS()
pdb_obj.fetch_data()