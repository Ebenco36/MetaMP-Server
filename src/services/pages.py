...
class Pages:
    ...
    def dashboard_helper_exemption(
        ...
        if not "*" in group_by_column and group_by_column != "":
            ...
            # Convert string list to list when the source column is present.
            if "rcsentinfo_software_programs_combined" in self.data.columns:
    ...
    def releasedStructuresByCountries(self, chart_dict):
        if isinstance(chart_dict, list):
            map_df = pd.DataFrame(chart_dict)
        else:
    ...
    def view_dashboard(self, get_query_params, conf=None, ranges_=None):
