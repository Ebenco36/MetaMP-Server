...
def get_number(text):
    ...
    try:
        ...
        logger.debug("Unable to extract number from %r: %s", text, ex)
        return None
...
def remove_html_tags(text):
    try:
        ...
        logger.debug("Unable to remove HTML tags from %r: %s", text, e)
        return ''
...
def replace_and_separate(text):
    ...
    try:
        ...
        logger.debug("Unable to normalize field prefix for %r: %s", text, e)
...
def add_uniprot_id(pdb_id):
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            logger.warning("UniProt ID not found for %s", pdb_id)
            return None
    else:
        logger.warning("Unable to fetch UniProt ID for %s", pdb_id)
        return None
...
def fetch_and_process_chloropleth_data(url):
    try:
        ...
        geometry_collection = data_.get("geometries", [])
        selected_data = []
        for dat in geometry_collection:
        ...
        logger.warning("Failed to fetch choropleth data from %s: %s", url, exc)
        return pd.DataFrame(columns=["location", "country", "count"])
...
def _build_country_reference_dataframe():
    rows = []
    for country in pycountry.countries:
        rows.append(
            {
                "flag": getattr(country, "flag", ""),
                "iso_code_2": getattr(country, "alpha_2", None),
                "iso_code_3": getattr(country, "alpha_3", None),
                "country": country.name,
                "country_number": getattr(country, "numeric", None),
                "country_with_flag": f"{getattr(country, 'flag', '')} {country.name}".strip(),
                "latitude": None,
                "longitude": None,
            }
        )
...
def countriesD():
    ...
    fallback_df = _build_country_reference_dataframe()
    ...
    for column in required_columns:
        if column not in country_df.columns:
            country_df[column] = fallback_df.get(column)

    return country_df[required_columns]
...
def getPercentage(df, unique_field="pdb_code", column="group"):
    ...
    # Prefer record-level de-duplication when the requested key exists, but
    # gracefully fall back to row-level counting for reduced summary tables.
    if unique_field and unique_field in working_df.columns:
        unique_df = working_df.drop_duplicates(subset=[unique_field]).copy()
    else:
        unique_df = working_df.copy()

    # Convert lists in the selected column to strings so mixed-value fields can
    # still be grouped for percentage summaries.
...
def get_published_date(doi):
    ...
    try:
        ...
        logger.warning("CrossRef payload missing published date for DOI %s: %s", doi, e)
    except ValueError as e:
        logger.warning("CrossRef date conversion failed for DOI %s: %s", doi, e)


# START group entries by year and for different databases
def load_and_prepare_data(data_list=None):
...
def get_data_by_countries(table_df):
    ...
    if len(available_columns) != len(required_columns):
        return pd.DataFrame(
            columns=["iso_code_2", "year", "count", "country", "country_number"]
        )

    grouped_df = table_df[required_columns].groupby(
    ...
    all_country_count = all_country_count.drop(
        columns=["location", "iso_code_3"],
        errors="ignore",
    )
