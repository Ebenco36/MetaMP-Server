...
def evaluate_dimensionality_reduction(data, methods_params):
    ...
    data_scaled = scaler.fit_transform(prepared_data.to_numpy())
...
class ClassifierComparison:
    ...
    def save_models(self, save_path='models/'):
        for clf_name, clf in self.models.items():
            joblib.dump(clf, f'{save_path}{clf_name}.joblib')

# Example usage:
# Assuming 'complete_numerical_data' and 'categorical_data' are predefined
# cc = ClassifierComparison(complete_numerical_data, categorical_data["group"])
...
class ClassifierComparisonSemiSupervised:
    ...
    def train_and_evaluate(self):
        ...
        y_combined = np.concatenate((self.y_train_labeled, -1 * np.ones(len(self.X_unlabeled), dtype=int)))
