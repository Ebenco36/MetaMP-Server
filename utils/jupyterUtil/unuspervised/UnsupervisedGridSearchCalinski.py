...
from GetFilePathIntoDict import getFiles
...
os.makedirs("clustering_results_calinski", exist_ok=True)
...
def perform_grid_search(model, param_grid, data_scaled):
    grid_search = GridSearchCV(model, param_grid, scoring=calinski_evaluate_cluster_quality, n_jobs=-1)
    grid_search.fit(data_scaled)
    best_model = grid_search
    return best_model
    
def fit_to_label(best_model, X, model_name, dataset_name, best_score):
    create_dir("./clustering_prediction_dataset_calinski/")
    data = X.copy()
    labels = best_model.fit_predict(X)
    data[model_name] = labels
    data.to_csv("./clustering_prediction_dataset_calinski/" + model_name + "_" + dataset_name + ".csv", index=False)
    saveChart(data, model_name, dataset_name, best_score)
    
def saveChart(X, labels, dataset_name, best_score):
    create_dir("./clustering_prediction_charts_calinski/")
    ...
    plt.savefig("./clustering_prediction_charts_calinski/" + f'{labels}_{dataset_name}_scatter_plot.png', dpi=300)
...
for data_path in data_paths:
    create_dir("./clustering_results_calinski/")
    ...
    results = {"Dataset": data_path, "Calinski Scores": {}}
    ...
    for algorithm_name, model, param_grid in clustering_algorithms:
        ...
        best_calinski = grid_search.best_score_
        
        fit_to_label(best_model, data_scaled, algorithm_name, data_path, best_calinski)

        # Store results
        results["Calinski Scores"][algorithm_name] = best_calinski
    ...
    # print(results)
    results_filename = os.path.join("clustering_results_calinski", f"results_{os.path.basename(data_path)}.json")
    #pd.DataFrame(results).to_json(results_filename, orient='records')
    saveJson(results, results_filename)

    # Create a bar chart for each dataset
    plt.figure(figsize=(10, 6))
    calinski = results["Calinski Scores"]
    plt.barh(list(calinski.keys()), list(calinski.values()), color='skyblue')
    plt.xlabel('Calinski Score')
    plt.title('Clustering Performance Chart')
    plt.xlim(-1, 1)  # Set the x-axis range to match silhouette score range
    plt.grid(axis='x', linestyle='--', alpha=0.6)

    # Save the chart with high quality
    plt.savefig("./clustering_results_calinski/" + f'{data_path}_scatter_plot.png', dpi=300)
