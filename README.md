...
- **/airflow**: Directory to save logs generated by Airflow.
- **/airflow_home**: Directory where DAG (Directed Acyclic Graph) implementations are stored.
- **/airflowConfig**: Contains the configuration files for Airflow.
- **/config**: Holds different environment configurations (e.g., development, testing, production).
- **/database**: Contains database connection classes and logic.
- **/datasets** and **/datasets_test**: Directories where processed datasets are stored for training/testing.
- **/logs**: Directory where application logs are saved.
- **/migrations**: Contains migration files for MetaMP.
- **/models**: Stores machine learning models.
- **/nginx**: Nginx configuration for setting up the Docker container.
- **/public**: Stores static content like images.
- **/serveConfig**: Configuration files required for Docker container startup.
- **/src**: Main application source code directory.
  - **/app**: Module containing the core of the application.
    - **/routes**: Defines Flask routes for each view.
    - **/views**: HTML and Jinja templates for rendering views.
    - **/services**: Business logic and services related to data processing.
    - **/models**: Database models used in the application.
- **/tests**: Contains test cases and scripts.
- **/utils**: Utility functions and helper scripts for MetaMP.
- **Dockerfile**: Docker configuration file for creating Docker images.
- **docker-compose.yml**: Docker Compose file for setting up a multi-container environment.
- **README.md**: Documentation for the MetaMP application.
- **requirements.txt**: Lists all Python dependencies.
- **env_example**: Example environment configuration file.
- **manage.py**, **app.py**, **server.py**: Python scripts for managing, running, and serving the application.
